...
void d_star_lite(Robot robot) {
    ...
    // Si no se encuentra un camino completo, buscar el punto más cercano
    int puntoCercanoX = -1;
    int puntoCercanoY = -1;
    int mejorCosto = INF;
    ...
    if (puntoCercanoX != -1 && puntoCercanoY != -1) {
        fprintf(stderr, "No se encontró un camino completo, pero el punto más cercano al destino es (%d, %d) con costo %d\n", puntoCercanoX, puntoCercanoY, mejorCosto);
        imprimirCamino((Punto){puntoCercanoX, puntoCercanoY}, celdaInfo);
        robot->pos->x = puntoCercanoX;
        robot->pos->y = puntoCercanoY;
