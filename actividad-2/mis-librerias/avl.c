...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato, 
  ...
  if(comp(raiz->dato, dato) == 0){
    AVL_Nodo* nueva_raiz;
    if(raiz->der){
      nueva_raiz = nodo_reemplazo(raiz->der);
...
/**
if (comp(raiz->dato, dato) < 0) { // el dato debe ir en el subarbol der
raiz->der = avl_nodo_insertar(raiz->der, dato, copia, comp);
//chequear balance
if (avl_nodo_factor_balance(raiz) == 2) {
  // casos 3 o 4
