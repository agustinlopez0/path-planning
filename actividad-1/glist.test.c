...
int main(){
  ...
  for (int i = 0; i < 6; ++i) {
    lista =
        glist_agregar_inicio(lista, contactos[i], (FuncionCopiadora)contacto_copia);
  ...
  GList lista_filtrada = glist_filtrar(lista, (FuncionCopiadora)contacto_copia, (Predicado)mayor_60);

  printf("\nLista filtrada:\n");
  glist_recorrer(lista_filtrada, (FuncionVisitante)contacto_imprimir);

  contacto_imprimir(contactos[1]);
  printf("%d\n", glist_include(lista_filtrada, contactos[2], (FuncionComparadora)contacto_comparar));
